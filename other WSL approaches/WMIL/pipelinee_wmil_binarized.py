...
counter_folder = 0
cc = 0
for folder in list(np.array(os.listdir(os.getcwd()))):#[[2,5,8]]):
    print(folder)
    os.chdir(arrays)
    ...
    for label in top_labels:
        ...
        with timer():
            ...
            print('ada value: ', ada_value, 'mom value: ', mom_value)
            ...
            # while sum(b) == 0 and counter_alfa < 10:
            #     #alfa += 0.01
            #     if counter_alfa > 0:
            #         ada_value = np.random.random_sample()
            #         mom_value = np.random.random_sample()
...
df.to_csv('wmil_results_9_100k_dynamic_model' + input3 + '.csv')
